...
- UPDATED pricing plans and help documentation: Enhanced homepage pricing to clearly show trial users are limited to 5 emails per day while Pro users get unlimited email sending. Updated help documentation with comprehensive email setup instructions, Brevo integration details, and added specific FAQs about email limitations and setup process. Documentation now aligns with the new email verification workflow and subscription-based email limits (August 2025).
- COMPLETED test welcome email feature for admin panel: Successfully implemented comprehensive test email functionality with Outlook-compatible HTML template using table-based layout. Features include admin-only access, email input validation, detailed error logging, and proper success/failure feedback. Test emails successfully delivered with improved cross-client compatibility and professional formatting (August 2025).
- SEPARATED admin panel subscription statistics: Updated admin panel to distinguish between paying subscribers (Stripe customers) and admin-granted free subscriptions. Statistics now show separate counts for paying subscribers vs admin subscriptions, with revenue calculations based only on paying customers (August 2025).
- FIXED mobile menu logo visibility: Updated hamburger menu header with dark background (slate-900) matching footer styling to ensure logo visibility in light mode. Enhanced close button contrast for better usability (August 2025).
